...
**Business model:** Free and open source. No license, no account, no purchase. Direct download.
...
- Skill data stays on-device; optional update checks are the only network call
...
Clove is free. Every feature is available with no license key, no account, and no purchase.
...
| "Is my skill data uploaded?" | Never. Skill scanning is on-device. Optional update checks are the only network call. |
